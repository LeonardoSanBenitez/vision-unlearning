...
In a FRESH process, regenerate one image per selected breed from the epoch-10 adapter (same prompt list,
same order, given --seed, batch_size=1) and compare pixel-for-pixel against the epoch-10 row already written
by make_epoch_grid.py (grid_dir/on_ep10_s{seed}_b{bi}.png). If the seed handling is correct, every pair is
identical (max abs pixel diff = 0). Writes sanity_reproduce_seed{seed}.json. Run with PYTHONPATH at the repo
root, HF_TOKEN set, HF_HUB_DISABLE_XET=1.
"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Cross-session reproducibility check for the epoch grid.")
    ...
    filenames = [f"repro_s{seed}_b{bi}.png" for bi in range(len(breeds))]
    logger.info("regenerating epoch-%d for %d breeds at seed %d (fresh process) ...", _EPOCH, len(breeds), seed)
    generate_dataset(
        model_base_name=_MODEL_ID, lora_name=adapter, prompts=prompts, output_path=str(sanity_dir),
        seeds=[seed], filenames=filenames, batch_size=1, lora_requires_inversion=False,
    )

    results: List[Dict[str, Any]] = []
    for bi, (name, _) in enumerate(breeds):
        grid_img = grid_dir / f"on_ep{_EPOCH}_s{seed}_b{bi}.png"
        repro_img = sanity_dir / f"repro_s{seed}_b{bi}.png"
    ...
    out = {
        "epoch": _EPOCH, "seed": seed, "n_breeds": len(breeds),
        "max_abs_pixel_diff_over_breeds": overall,
        "all_pixel_identical": all(r["identical"] for r in results),
        "per_breed": results,
    }
    (_OUT / f"sanity_reproduce_seed{seed}.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
    print("SANITY_OK seed=%d all_identical=%s max_abs=%d" % (seed, out["all_pixel_identical"], overall))
    return 0
