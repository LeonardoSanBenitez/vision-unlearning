...
# process (the one-pipeline-per-process constraint; run_campaign.py itself refuses a comma-separated
# --epochs list for the same reason). No new logic lives here.
#
# Finishes the remaining seed-42 checkpoints, then runs seed 43 in full (train + all generation).
#
...
echo "=== seed 42 remaining checkpoints ==="
...
echo "=== seed 43 training ==="
echo "--- seed 43 train : $(date +%H:%M:%S) ---"
"$PY" run_campaign.py --stage train --seed 43

echo "=== seed 43 generation (off + all 13 checkpoints) ==="
