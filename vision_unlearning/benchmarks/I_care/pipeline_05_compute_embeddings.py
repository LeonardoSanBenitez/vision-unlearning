...
if __name__ == "__main__":
    ...
    for index in range(index_start, index_start + max_identities):
        ...
        #
        # The baseline embedding output file is method-agnostic:
        #   embeddings_{task}_original.json
        # The lora-OFF images come from the shared task-level baseline folder (produced by
        # 0_generate_dataset_original.py), so the baseline embedding depends only on the task
        # — never on the unlearning method or its epoch count.  get_embedding_output_path
        # ignores the method/epochs arguments for target_preprocessed == "original" and emits
        # this single file, which every method run reads.  (The pre-refactor per-method name
        # embeddings_{task}_original_{method}_{epochs}.json has been retired everywhere.)
