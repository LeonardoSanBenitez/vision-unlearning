"""
3_compute_embeddings.py — Compute DINOv2 embeddings for generated image datasets.
...
Dependencies (all in sd-interpretability venv):
    torch (2.9.1+rocm), torchvision, huggingface_hub, pillow, numpy, python-dotenv, requests

vision_unlearning is imported directly from ../vision-unlearning (added to sys.path below).

HF_TOKEN: loaded from .env in this directory or from ../forgety/.env via dotenv.

Usage:
    python 3_compute_embeddings.py --task people --method distil --max-identities 20
    python 3_compute_embeddings.py --task people --method uce --max-identities 20
    python 3_compute_embeddings.py --dry-run --task people --method distil --max-identities 100
    python 3_compute_embeddings.py --help
"""
...
def get_manifest_entry(
    ...
    dataset_folder = get_generated_dataset_folder(
        task, method, num_train_epochs, target_hf_name, base_folder=base_folder
    )
...
if __name__ == "__main__":
    ...
    assert len(HF_TOKEN) > 0, (
        "HF_TOKEN not found. Create a .env file in unlearning-analysis/ "
        "or ensure ../forgety/.env exists."
    )
    ...
    for index in range(index_start, index_start + max_identities):
        ...
        #
        # NOTE on filename coupling (Cidral review 2026-05-23, Issue 2):
