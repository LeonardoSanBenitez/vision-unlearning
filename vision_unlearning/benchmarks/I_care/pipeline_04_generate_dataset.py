...
from vision_unlearning.datasets.testbed import get_target_overwrite, get_generated_dataset_folder, get_unlearned_model_folder, exists_unlearned_model, exists_unlearned_dataset
...
index_start: int = 0
max_identities: int = 100
task: Literal['scenes', 'objects', 'breeds', 'people'] = 'people'  # half-way thoruhg
method: Literal['munba', 'uce', 'distil'] = 'munba'
...
for index in range(index_start, index_start + max_identities):
    ...
    if perform_unlearning and replace_if_exists or not exists_unlearned_model(task, method, num_train_epochs, target):
    ...
    if perform_dataset_generation:
        ...
        generated_dataset_output_path = get_generated_dataset_folder(task, method, num_train_epochs, target_preprocessed)
    
        if replace_if_exists or not exists_unlearned_dataset(generated_dataset_output_path, generate_dataset_seeds, prompts):
            logger.info(f'Generating images for unlearning "{target_preprocessed}" into folder "{generated_dataset_output_path}"')
            for seed in generate_dataset_seeds:
                for lora_state in ['on', 'off']:
                    # TODO: print status update
                    filenames = [f'{lora_state}_{seed}_{p}.png' for p in prompts]
                    if method == 'uce':
                        model_generate_name = None  #output_dir if (lora_state == 'on') else model_base_name
                        lora_name = None
        ...
        assert exists_unlearned_dataset(generated_dataset_output_path, generate_dataset_seeds, prompts)
