...
def load_style_generated_images(path, exclude="Abstractionism"):
    ...
    """
    image_paths = []
    images_generated = os.listdir(path)
    ...
    for theme in theme_tested:
        for object_class in class_tested:
            ...
            imgs_filtered = [img for img in images_generated if img.startswith(f"{theme}_{object_class}_")]
            
            for individual in imgs_filtered:
                image_paths.append(os.path.join(path, individual))
    ...
    for theme in theme_tested:
        for object_class in class_tested:
            for idx in range(1, 6):
                image_paths.append(os.path.join(path, theme, object_class, f"{idx}.jpg"))
