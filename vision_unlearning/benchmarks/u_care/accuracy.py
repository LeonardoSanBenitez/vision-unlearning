...
def compute_accuracy(args, forget_theme):

    device = "cuda" if torch.cuda.is_available() else "cpu"
    input_dir = os.path.join(args.input_dir, forget_theme)

    # Create folder if not exist
    os.makedirs(args.output_dir, exist_ok=True)
    output_path = os.path.join(args.output_dir, f"{forget_theme}_{args.task}_eval.pth")
    ...
    results = {}
    results["test_theme"] = forget_theme
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, required=True, help='Path to classifier checkpoint, the classifier should match the task!')
    ...
    for forget_theme in args.forget_concept:
        if forget_theme not in all_possibilities:
            print(f"Error: The concept '{forget_theme}' is not in the available themes or classes list.")
            continue
        if forget_theme == "Seed_Images":
            print(f"Skipping evaluation for concept '{forget_theme}' as it is not applicable.")
            continue
        compute_accuracy(args, forget_theme)