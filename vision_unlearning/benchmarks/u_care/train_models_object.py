...
def train_model_object(model_name_or_path,
                      dataset_root,
                      theme_name,
                      output_dir,
                      seed=42,
    ...
    try:
        forget_class_index = class_available.index(target)
    except ValueError:
        print(f"Error: The target class '{target}' is not in the available classes list.")
        return None
    
    try:
        overwrite_class_index = class_available.index(target_overwrite)
    except ValueError:
        print(f"Error: The target class '{target_overwrite}' is not in the available classes list.")
        return None
    ...
    validation_prompt = f"An image of {overwrite_class} in Crayon style."
...
if __name__ == "__main__":
    ...
    parser.add_argument('--classes', type=str, nargs="+", default=class_available, required=False, help="Forget class list.")

    args = parser.parse_args()
    
    for target in args.classes:

        if target not in class_available:
            print(f"Error: The target class '{target}' is not in the available classes list.")
            continue
        ...
        model_base_path = os.path.join(args.output_dir, f"forget_class_{target}")
        os.makedirs(model_base_path, exist_ok=True)

        result = train_model_object(
            model_name_or_path=args.pretrained_model_base_path,
            dataset_root=args.dataset_base_path,
            theme_name=target,
            output_dir=model_base_path,
            target=target,
            target_overwrite=overwrite_class,
        ...
        if result is not None:
            print(f"Finished training for forgetting class: {target}")
        else:
            print(f"Training for forgetting class: {target} was skipped or failed.")
