    ...
    for layer_num in tqdm(range(len(projection_matrices))):
        ...
        with torch.autocast("cuda",enabled=False):
            ...
            with torch.no_grad():
                ...
                weight = projection_matrices[layer_num].weight.detach().float()
                mat1 = float(lamb) * weight

                # mat2 = \lambda I + \sum{k k^T}
                mat2 = float(lamb) * torch.eye(weight.shape[1],device=weight.device, dtype=torch.float32,) 
                ...
                for cnt, t in enumerate(zip(old_texts, new_texts)):
                    ...
                    for_mat1 = (value_vector.float() @ context_vector_T.float()).sum(dim=0)
                    for_mat2 = (context_vector.float() @ context_vector_T.float()).sum(dim=0)
                ...
                updated_weight = torch.linalg.solve(mat2.T,mat1.T).T

                if not torch.isfinite(updated_weight).all():
                    raise RuntimeError(f"Non-finite UCE update at layer {layer_num}")

                projection_matrices[layer_num].weight = torch.nn.Parameter(
                    updated_weight.to(projection_matrices[layer_num].weight.dtype)
                )
