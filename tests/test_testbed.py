...
# ``TestGetTargetPreprocessedCharacterization`` stood here. It locked the pre-fix output of
# ``get_target_preprocessed`` -- including its people/breeds no-ops -- as a "before" snapshot for a
# deferred repair. The repair landed: the rule now lives in ``vision_unlearning.datasets.entity_names``
# and is covered by ``tests/test_entity_names.py``, which runs in the lite tier (this file does not --
# ``tests/conftest.py`` skips it when torch is absent) and characterises the transform over all 300
# entities of the three tasks instead of six hand-picked ones.
